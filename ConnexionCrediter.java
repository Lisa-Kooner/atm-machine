...
public class ConnexionCrediter extends JFrame{
	...
	private final JTextArea textConsigne = new JTextArea("Écrivez les informations reliées au compte  \nsur "
			+ "lequel, vous voulez ajouter de l'argent.");
	...
	//Constructeur
	ConnexionCrediter(ArrayList<Client> listeCompte){
		...
		textConsigne.setBounds(56, 5, 230, 40);
		textConsigne.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5)); 
					...
					if(numCompte.matches("\\d+") && numCompte.length()==4) {
						for(Client c : listeCompte) {
							...
							if(c.getNumeroCompte()==numeroCompte && c.getMotPasse().equals(motPasse)) {
								Crediter crediterFenetre = new Crediter(numeroCompte, listeCompte);
								dispose();
								break;
							}
						}
